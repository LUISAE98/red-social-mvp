...
type CommunityBase = Omit<
  Community,
  "postsCount" | "livesCount" | "memberCount" | "creatorName" | "creatorAvatar" | "creatorHandle"
>;

function toCommunityBase(id: string, d: FireCommunity): CommunityBase {
  return {
    id,
    name: d.name ?? null,
...
export default function PrivateCommunitiesPage() {
  ...
  const filtered = communities.filter((c) =>
    tab === "active" ? !c.isDeleted : c.isDeleted,
  );

  const activeCount = communities.filter((c) => !c.isDeleted).length;
  const deletedCount = communities.filter((c) => c.isDeleted).length;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border: `1px solid ${isSelected ? "#7c3aed" : c.isDeleted ? "#2a0a0a" : "#1a1a1a"}`,
                  borderRadius: 10,
                  padding: "14px",
                  cursor: "pointer",
                ...
                onMouseEnter={(e) => {
                  if (!isSelected) {
                    (e.currentTarget as HTMLDivElement).style.borderColor = c.isDeleted ? "#3d1515" : "#2d2d2d";
                  }
                }}
                onMouseLeave={(e) => {
                  if (!isSelected) {
                    (e.currentTarget as HTMLDivElement).style.borderColor = c.isDeleted ? "#2a0a0a" : "#1a1a1a";
                  }
                }}
              >
                {/* Top: avatar + title */}
