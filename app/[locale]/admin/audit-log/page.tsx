    ...
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#fff", margin: 0 }}>
          {tAdmin("auditLogTitle")}
        </h1>
        <p style={{ fontSize: 13, color: "#555", margin: "6px 0 0" }}>
          {tAdmin("auditLogDescription")}
        </p>
      </div>

      {loading ? (
        <div style={{ color: "#555", fontSize: 14 }}>{tAdmin("auditLogLoading")}</div>
      ) : reports.length === 0 ? (
        <div style={{ color: "#555", fontSize: 14 }}>{tAdmin("auditLogEmpty")}</div>
      ) : (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: "#444", marginTop: 3 }}>
                  {tAdmin("auditLogResolved", { time: r.resolvedAt ? rel(r.resolvedAt) : "—" })}
                </div>
              </div>
