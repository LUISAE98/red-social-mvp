...
export default function VibraFlameIcon({
  ...
  const bodyActiveId = `vibraFlameBodyActive-${safeId}`;
  const tipActiveId = `vibraFlameTipActive-${safeId}`;
  const coreActiveId = `vibraFlameCoreActive-${safeId}`;
  const bodyInactiveId = `vibraFlameBodyInactive-${safeId}`;
  const tipInactiveId = `vibraFlameTipInactive-${safeId}`;
  const bodyPremiumId = `vibraFlameBodyPremium-${safeId}`;
  const tipPremiumId = `vibraFlameTipPremium-${safeId}`;
  const corePremiumId = `vibraFlameCorePremium-${safeId}`;
    ...
    >
      ...
      <defs>
        ...
        >
          ...
          <stop offset="74%" stopColor="#c084fc" />
      ...
      <g className={active ? "vibra-flame-like-pop" : undefined}>
        <path
          ...
          fill={active ? `url(#${premium ? bodyPremiumId : bodyActiveId})` : `url(#${bodyInactiveId})`}
        />
        ...
        <path
          ...
          fill={active ? `url(#${premium ? tipPremiumId : tipActiveId})` : `url(#${tipInactiveId})`}
        ...
        <path
          ...
          fill={active ? `url(#${premium ? corePremiumId : coreActiveId})` : "#d1d5db"}
