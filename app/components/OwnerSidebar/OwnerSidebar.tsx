...
// Tipos y sub-componentes extraídos; re-exportados para consumidores externos.
export * from "./OwnerSidebar.parts";
import {
  type OwnerSidebarCache,
  type ExclusiveSessionRequestDoc, type FollowedProfileLite,
  type GreetingRequestDoc, type GroupDocLite, type JoinRequestRow,
  type MeetGreetRequestDoc, type OutgoingJoinRequestRow, type TopView,
  type UserDoc, type UserMini,
} from "./OwnerSidebar.parts";

// Cache mutable de módulo (se reasigna en este componente): vive aquí, no en parts.
...
const handleOwnerSidebarPullRefresh = useCallback(async () => {
  ...
  // Bandeja de DM. Solicitudes en query aparte para que un aluvión de
  // desconocidos no desplace las conversaciones reales del inbox.
  const {
    conversations,
...
const groupsForSeen = [
  ...
  const relevantUserIds = useMemo(() => {
    ...
    if (viewer?.uid) {
      [...conversations, ...conversationRequests].forEach((conv) => {
...
.profile-owner-sidebar-panel {
  ...
  overflow: hidden;
  width: 100%;
  max-height: calc(var(--vb-alto-pantalla) - 100px);
  padding: 10px;
...
className="profile-owner-sidebar-fixed"
  style={{
    ...
    maxHeight: `calc(var(--vb-alto-pantalla) - ${ui.sidebarTop + ui.sidebarBottom}px)`,
    zIndex: 9998,
    fontFamily: fontStack,
    color: "#fff",
  }}
