...
async function liberarHoldMuerto(
  ...
  try {
    ...
    if (canceled) {
      ...
      logger.info("hold_liberado_sin_entrega", { id, estado, sourceType: "meetGreetRequest" });
      return;
    }
...
// devolución → crédito). Las AGENDADAS que no se realizan ya las cubre el no-show.
...
export async function autoRejectUndeliveredMeetGreetRequestsHandler(): Promise<number> {
  ...
  logger.info("auto_reject_undelivered_meet_greet", { rejected: n, scanned: snap.size });
  return n;
}
