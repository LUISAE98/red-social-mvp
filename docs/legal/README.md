...
> (Stripe Connect, Merchant of Record). Se mantienen en pausa hasta que el **money-out / payout esté
...
| G13 | **Persistir evidencia fiscal por transacción (indicios 18‑C)** | 🔴 | T&C §43 / Aviso #3 / fiscal §3 | Como vendedor directo, guardar por venta los indicios de país del Comprador (IP, banco/BIN del medio de pago, domicilio, código telefónico) como evidencia de la determinación del impuesto. Hoy el país viene del cliente (cookie `vibra_country`); falta determinación autoritativa en backend + persistencia. |
| G14 | **Factura/comprobante de venta al Comprador (seller of record)** | 🔴 | T&C §43 / fiscal §0 | Vibra debe emitir el comprobante/factura de la venta al Comprador (no el creador). Falta el flujo de facturación como vendedor por país. |
