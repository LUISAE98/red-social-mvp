...
| 2 | **Acuerdo de Creador / Monetización** | 🟨 | ⭐ | Al activar monetización (`ProfileServicesTab`) + antes del 1er retiro | [`02-acuerdo-de-creador.md`](./02-acuerdo-de-creador.md) |
...
| G5 | **Consentimiento bilateral de grabación antes de la videollamada** | ⏸️ diferido | T&C §51.2 / doc #10 | **Ya NO es bloqueante para lanzar:** los únicos productos de videollamada 1‑a‑1 grabada (*Sesión exclusiva* y *Tiempo contigo*, antes "meet & greet") están **bloqueados/pendientes** (T&C §47.3). Se retoma cuando se habiliten esos productos: modal bloqueante de consentimiento de ambas partes antes de entrar a `LiveKitVideoRoom`. |
...
| G13 | **Persistir evidencia fiscal por transacción (indicios 18‑C)** | 🔴 | T&C §43 / Aviso #3 / fiscal §3 | Guardar por venta los indicios de país del Comprador (IP, banco/BIN del medio de pago, domicilio, código telefónico) como evidencia de la determinación del impuesto **y del sustento de la exportación a 0%**. Hoy el país viene del cliente (cookie `vibra_country`); falta determinación autoritativa en backend + persistencia. |
| G14 | **Los tres comprobantes del modelo de intermediación** | 🔴 | T&C §36 / fiscal §0.3 | (1) venta al Comprador, que **Vibra emite por cuenta del creador** con el sello digital de éste; (2) comisión de Vibra al creador mexicano, 25% + IVA; (3) constancia periódica de retenciones. Creador no mexicano: solo comprobante de pago. 🔴 Incluye la **factura global**, que bajo intermediación deja de ser opcional. |
| G15 | **Acceptance copy de "donación / apoyo"** | 🔴 | T&C §55 / fiscal §0.4 | En el flujo de propina/donación, mostrar y registrar el texto de aceptación: el apoyo es voluntario y **no garantiza contraprestación**; si desbloquea acceso, se cobra como el Servicio correspondiente. |

## Bitácora de avance

| Fecha | Documento | Cambio |
|-------|-----------|--------|
| 2026-08-26 | ⚠️ CAMBIO DE MODELO (definitivo) | **Vibra vuelve a INTERMEDIARIO**, ahora con confirmación de fiscalista. Se revierte el paréntesis de *vendedor directo* (28-jul a 25-ago-2026). El creador vende y presta; Vibra intermedia y cobra por su cuenta con **mandato de cobro**. Actualizados: **#1 T&C → v0.6** (§16 reescrita como intermediación, definiciones creador=vendedor, §34 el creador fija precio, §35 comisión + IVA por encima, §36 retenciones y 3 comprobantes, §37.1 contratas con el creador, §43 impuesto por país del comprador con **0% de exportación confirmado**, §44 deducciones, §47.1 los 11 servicios, §47.2 licencia del creador vía Vibra, §47.3/§51/§77 **desbloqueados** Sesión exclusiva y Tiempo contigo, §55 **apoyos = contraprestación, no donativos**, KYC sin Didit). **#2 Acuerdo de Creador → v0.3 reescrito completo** (nuevo §3 **mandato de cobro con aceptación separada**, §7 los 3 comprobantes + sello digital del creador mexicano, §11 licencia de acceso con aceptación específica para contenido de pago). **#7 Reembolsos → v0.3** (Vibra gestiona por ser quien cobró, sin ser vendedora). **Fiscal → §0 reescrita** con la matriz de 4 combinaciones y las tasas 2026; **§5 vuelve a regir**. `modelo-financiero.md`, `marco-legal.md` y `facturacion-pendientes.md` actualizados. 🔴 Nuevo bloqueante: **la factura global deja de ser opcional**. |
...
| 2026-07-28 | Bloque 3 en espera | **#8 Wallet y #9 Pagos/Retiros pausados** hasta elegir e integrar la procesadora (Pagsmile/EBANX, D‑01). Dependen de si la procesadora recauda/remite el impuesto y de la mecánica de payouts. Se retoman tras la integración. Draftables sin procesadora: #11 Biométrico (KYC, bloqueante) y #12 DMCA. |
| 2026-07-31 | ⚙️ Stripe + 25% | **Procesadora definitiva = Stripe** (Stripe Connect, Vibra = Merchant of Record) y **comisión 25% (reparto 75/25)**. Propagado: instructivo fiscal (limpiado 77/23→75/25, dLocal/MP/Pagsmile/EBANX→Stripe, D‑01 resuelto), **#1 T&C → v0.5** (Stripe nombrado, §35 75/25, §34/§40 disclosure de cargo fijo + FX/Adaptive Pricing), **#2 Acuerdo → v0.2** (75/25, Stripe transfer, nota Didit→Stripe), **#3 Aviso** (Stripe como encargado + nota Didit→Stripe). **Didit se reemplazará por Stripe antes de producción** (podría quedar para edad de comprador + duplicados). **#8/#9 siguen en pausa** pero ahora por money-out pendiente, no por procesadora. Fuentes: `docs/modelo-financiero.md`, `docs/stripe-integracion.md`. |
| 2026-07-28 | Aclaración productos | **"Meet & greet" se renombró a "Tiempo contigo" y está BLOQUEADO.** Por tanto **no hay ningún producto de videollamada 1‑a‑1 grabada disponible** (Sesión exclusiva y Tiempo contigo, ambos parkeados). Parkeadas T&C §51 y §77, #7 §2.2, y ajustado el dato de grabaciones en el Aviso #3. La brecha **G5** (consentimiento de grabación) pasa a **diferida**: ya no es bloqueante para lanzar. |
| 2026-07-28 | ⚠️ CAMBIO DE MODELO | Vibra pasó de **INTERMEDIARIO** a **VENDEDOR DIRECTO (seller of record)**. Auditoría + actualización de documentos afectados: **#1 T&C → v0.4** (§16 vendedor, definiciones creador=proveedor/comprador↔Vibra, §33/§34/§35 reparto 77/23, §43 impuesto por país del comprador, §47.2 licencia al comprador, §47.3 + §51 parkeo de *Sesión exclusiva*/*Tiempo contigo*, §55 donaciones, procesador genérico). **#7 Reembolsos → v0.2** (Vibra responde al consumidor). **#3 Aviso → +finalidad fiscal por país del comprador + evidencia 18‑C + procesador genérico**. **#2 Acuerdo de Creador → v0.1 redactado bajo el nuevo modelo** (creador=proveedor/licenciante, licencia a Vibra, reparto, indemnización a Vibra, KYC, independencia). Nuevas brechas **G13–G15**. **#8 y #9 se redactarán nativamente bajo el modelo vendedor.** Pendiente validación abogado+fiscalista. |
