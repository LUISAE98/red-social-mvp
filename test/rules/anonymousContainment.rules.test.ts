...
describe.skip("B4-M04 — la colección posts no acepta escrituras directas del cliente", () => {
  ...
  it("un post de perfil propio y legítimo tampoco pasa ya por el cliente", async () => {
    await seedAll([[`users/${REAL}`, profile(REAL, "realuser")]]);
    await assertFails(
      setDoc(doc(realDb(), "posts/p_perfil_srv"), {
        authorId: REAL,
        profileId: REAL,
        contextType: "profile",
        isDeleted: false,
  ...
  it("tampoco uno de comunidad, ni siendo miembro activo", async () => {
    await seedAll([
      [
        `groups/${GRUPO_AJENO}`,
        { ownerId: "otra_persona", visibility: "public", isActive: true, postingMode: "members" },
      ],
      [
        `groups/${GRUPO_AJENO}/members/${REAL}`,
        { userId: REAL, roleInGroup: "member", status: "active" },
      ],
      [`users/${REAL}`, profile(REAL, "realuser")],
    ]);
    await assertFails(
      setDoc(doc(realDb(), "posts/p_grupo_srv"), {
        authorId: REAL,
        groupId: GRUPO_AJENO,
        contextType: "group",
        isDeleted: false,
